...
function Position() {
  ...
  const handleUpdateRank = async (RankId) => {        //직 수정
    try {
  ...
  const handleEditClick = (rank) => {   //직 목록에 수정 버튼 클릭 시 해당 직 정보 받아오기
  ...
  useEffect(() => {       //직급 목록 사용 여부 구분
    const fetchPositions = async () => {
      try {
        const response = await axios.get('/api/v1/getRanks');
        if (Array.isArray(response.data.data)) {
          const filteredRanks = filterFlag === -1 ? response.data.data : response.data.data.filter(rank => rank.rankFlag === filterFlag);
  ...
  return (
    <RankContainer>
      ...
      <FlexStyle>
        ...
        >
          <option key={ranks.rankFlag} value={1}>사용함</option>
          <option key={ranks.rankFlag} value={0}>사용 안 함</option>
          <option key={ranks.rankFlag} value={-1}>전체</option>
        </select>
        <ButtonPosition>
                  ...
                  <Form style={{ width: "130px" }}>
                    ...
                    <InputGroup>
                      ...
                      {isEditing && (
                        <Select
                          value={flag}
                          onChange={(e) => { setRankFlag(e.target.value) }}
                        >
                          <option key={rank.rankFlag} value={1}>사용함</option>
                          <option key={rank.rankFlag} value={0}>사용 안 함</option>
                        </Select>
                      )}
                    </InputGroup>
                    <TotalBtn>
                      <Button onClick={closeModal}>취소</Button>
                      <Button onClick={isEditing ? handleUpdateRank : handleSaveRank}>{isEditing ? '수정' : '저장'}</Button>
                    </TotalBtn>
                  </Form>
                </div>
              </Modal>
