...
function AdminRequest() {
    ...
    const { timename, startTime, endTime, memo, startTimeChange, endTimeChange, changeName, changeMemo } = inputs;
    ...
    const TimeDataAdd = async () => {
        new workTimeUnit().post(timename, startTime, endTime, memo).then((TimeDataAdd) => {
    ...
    const TimeDateChange = async () => {
        const idx = value.idx
        new workTimeUnit().put(idx, changeName, startTimeChange, endTimeChange, changeMemo).then((TimeChange) => {
            ...
            case "default":
                return (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                        <Input value={timename} name="timename" type="text" onChange={handleChange} placeholder={"근무 시간 이름"} />
                        <Input value={startTime} name="startTime" onChange={handleChange} placeholder={"출근시간"} />
                        <Input value={endTime} name="endTime" type="text" onChange={handleChange} placeholder={"퇴근시간"} />
                        <Input value={memo} name="memo" type="text" onChange={handleChange} placeholder={"설명"} />
                        <Button name="button" type="button" onClick={TimeDataAdd}>추가하기</Button>
                    </div>
                );
            case "custom":
                ...
                return (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                        <select onChange={IdxCheck}>
                            ...
                            {timeDataCopy && timeDataCopy.map((TimeCheck, index) => (
                                <option key={index} value={TimeCheck.idx}>{TimeCheck.startTime + "~" + TimeCheck.endTime}</option>
                            ))}
                        </select>
                        <Input value={changeName} name="changeName" type="text" onChange={handleChange} placeholder={value === undefined ? "근무 시간 이름" : value.name} />
                        <Input value={startTimeChange} name="startTimeChange" onChange={handleChange} placeholder={value === undefined ? "출근시간" : value.startTime} />
                        <Input value={endTimeChange} name="endTimeChange" onChange={handleChange} placeholder={value === undefined ? "퇴근시간" : value.endTime} />
                        <Input value={changeMemo} name="changeMemo" type="text" onChange={handleChange} placeholder={value === undefined ? "설명" : value.memo} />
            ...
            case "delete":
                ...
                return (
                    <div style={{ width: "100%", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center" }}>
                        ...
                        <input type="text" defaultValue={value !== undefined ? `${value.startTime} ~ ${value.endTime}` : ""} />

                        <button onClick={() => { TimeDateDelete() }}>근무시간 삭제</button>
                    </div>
