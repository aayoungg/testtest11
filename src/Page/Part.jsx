...
const Part = () => {
  ...
  useEffect(() => {       //부서 목록 사용 여부 구분
    const fetchParts = async () => {
      try {
        const response = await axios.get('/api/v1/getParts');
        if (Array.isArray(response.data.data)) {
          const filteredParts = filterFlag === -1 ? response.data.data : response.data.data.filter(part => part.partFlag === filterFlag);
  ...
  return (
    <PartContainer>
      ...
      <FlexStyle>
        ...
        >
          <option key={parts.partFlag} defaultValue={1}>사용함</option>
          <option key={parts.partFlag} defaultValue={0}>사용 안 함</option>
          <option key={parts.partFlag} defaultValue={-1}>전체</option>
        </select>
        <ButtonPosition>
                  ...
                  <Form style={{ width: "130px" }}>
                    ...
                    <InputGroup >
                      ...
                      {isEditing && (
                        ...
                        >
                          <option key={part.partFlag} value={1}>사용함</option>
                          <option key={part.partFlag} value={0}>사용 안 함</option>
                        </Select>
                      )}
                    </InputGroup>
                    <TotalBtn>
                      <Button onClick={handleCancelEdit}>취소</Button>
                      <Button onClick={isEditing ? handleUpdatePart : handleSavePart}>{isEditing ? '수정' : '저장'}</Button>
                    </TotalBtn>
                  </Form>
                </div>
              </Modal>
