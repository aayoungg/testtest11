...
function Leftmenu({ workdataState }) {
    ...
    useEffect(() => {
        const check = async () => {
            if (Cookiecheck == formattedDate) {
                try {
                    const workdata = await new workcheck().get(userIdx, startDate, endDate, type);
    ...
    const todayButtonTime = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 11, 59, 59); // 오늘 오전 9시
    console.log(todayButtonTime)
    useEffect(() => {


        // 오늘 오전 9시와 내일 오전 6시의 시간 계산
        const todayNineAM = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 9, 0, 0); // 오늘 오전 9시
        const tomorrowSixAM = new Date(tomorrow.getFullYear(), tomorrow.getMonth(), tomorrow.getDate(), 6, 0, 0); // 내일 오전 6시
        if (now >= todayNineAM && now < tomorrowSixAM) {
            { WorkData == undefined ? setLateText(nowDate + "일 지각") : WorkData.startDate > Logindata.data.startTime ? setLateText(nowDate + "일 지각") : setLateText("") }
        }
    ...
    const menuList = [
        ...
        ...(rankName === "팀장" || rankName === "관리자"
            ? [{
                ...
                name: rankName === "관리자" ? "직원 근무 시간 생성" : "직원 근무 시간 수정",
                link: rankName === "관리자" ? "/work/AdminRequest" : "/work/CreateWork",
            }]
            : [])
    ];
        ...
        <>
            <MenuLeft>
                <LateContent>
                    ...
                    {/* 출근 */}
                    {/* 출근 퇴근 부분 함 확인 요망 */}
                    <Late>{workdata !== "undefined" && workdata !== undefined ? "출근시간 " + workdata.startDate.split("T")[1].split(":").slice(0, 2).join("시") + "분" : ""}</Late>
                    {/* 퇴근 */}
                    <Late>{workdata !== "undefined" && workdata !== undefined && workdata.endDate !== null ? "퇴근시간 " + workdata.endDate.split("T")[1].split(":").slice(0, 2).join("시") + "분" : ""}</Late>
                </LateContent>
                {/* <Late>{leaveText}</Late> */}

                {workdata && workdata.length === 0 || workdata === undefined || workdata.endDate == null ? <CommuteBtn onClick={toggleModal}>{workdata && workdata.length === 0 || workdata == undefined || workdata.startDate == null ? "출근하기" : "퇴근하기"}</CommuteBtn> : "퇴근 하셨습니다."}
                {isModalOpen && <Modal IsWorkData={setWorkdata} onClose={toggleModal} />}

                {menuList.map((menu) => (
                    <div key={menu.id}>
                        <NavStyle
                            to={menu.link}
                            onClick={() => { menu.subMenu != null && menu.subMenu.length > 0 && toggleContentVisibility(menu.id) }}
                            style={{ background: pathname == menu.link ? '#ECF2FF' : "#FFFFFF", color: pathname == menu.link ? '#0055FB' : '#000000' }}
                        >
